...
class ControlStdout: public CRTSController
{
    public:

        ControlStdout(int argc, const char **argv);

        ~ControlStdout(void) {  DSPEW(); };
...
ControlStdout::ControlStdout(int argc, const char **argv): nextBytesIn(BYTES_PER_CHANGE)
{
    ...
    if(c == 0) throw "could not get Rx Control";
...
void ControlStdout::start(CRTSControl *c, uint32_t numIn, uint32_t numOut)
{
...
void ControlStdout::stop(CRTSControl *c)
{
    DSPEW();
}


void ControlStdout::execute(CRTSControl *c, const void *buffer, size_t len, uint32_t channelNum)
{
...
CRTSCONTROLLER_MAKE_MODULE(ControlStdout)
