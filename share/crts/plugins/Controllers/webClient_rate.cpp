...
"                      default PERIOD is %g.  This period is used to throttle the\n"
"                      rate at which filter parameters are written to the web\n"
"                      clients.  Only the last values set are sent to the clients\n"
"                      when the timer says it's time to send.\n"
"\n"
"\n"
...
class Client: public CRTSController
{
    ...
    private:
        ...
        // the web server.
        //
        // This is called in the flow stream every time a packet goes from
        // one filter to another.  The socket write call take an order of
                ...
                const std::string parameterName, double value)
        {
            ...
            // Add this event to the queue.  If there is an
            // event with this name (str) already than this
            // just overwrites it, which is fine.
...
static
std::string liquidSync_str = "I{\"name\":\"updateParameter\",\"args\":["
                "\"liquidSync\",\"totalBytesOut\",";
...
static void *parameterSender(Client *client) {
    ...
    while(runThread)
    {
        ...
        // Part of a "Terrible Hack"
        if(gotSyncBytes == false) {
            std::string str = liquidSync_str;
            str += "0.0]}\004";
            socket.send(str.c_str());
        }
