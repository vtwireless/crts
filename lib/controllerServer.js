...
function onConnect(socket) {
    ...
    var programName = 'program_' + connectionCount++;
    ...
    socket.on('close', function() {
        ...
        if(controllers[programName] !== undefined)
            delete controllers[programName];
    ...
    function(wsIo, wsId, filterName=null, parameterName=null) {
        ...
        console.log("webSocket(" + wsId + ') subcripting to: ' +
            filterName + ":" + parameterName);
    ...
    io.On('controlList', function(set, get, image) {
            ...
            controlNames.forEach(function(controlName) {
                ...
                obj[controlName].forEach(function(parameterName) {
                    ...
                    Object.keys(opt.users).forEach(function(userName) {
                        // freq: { joe: true/*can access*/}
                        control[parameterName][userName] = false;
                    });
                });
            });
        }
        ...
        // Create empty 'get' subscription lists if they to not exist
        ...
        Object.keys(get).forEach(function(filterName) {
            if(controller.getSubscribers[filterName] !== undefined)
                return;
            controller.getSubscribers[filterName] = {};
            Object.keys(get[filterName]).forEach(function(parameterName) {
    ...
    io.On('getParameter', function(filterName, parameterName, value) {

        // This is the parameter change event that sends data
        // to the browsers webSocket.  They did not poll in this case.
        // They registered a callback for the parameter change event.
        //
        // TODO: rename this event, change 'getParameter'.
