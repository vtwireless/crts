...
function onConnect(socket) {
    ...
    controller.setParameter = function(controlName, parameterName, value) {
        ...
        // Make sure that value is a float.  This was causing a bug by
        // passing a string and not a number.
        value = parseFloat(value);

        try {
            // Constrain the value between min and max
            // and check that the parameter exists in our list.
            let min = controller.set[controlName][parameterName].min;
            let max = controller.set[controlName][parameterName].max;
            if(value < min)
                value = min;
            else if(value > max)
                value = max;
        } catch(ee) {
            spew("setParameter invalid parameter: " + programName + ":" +
                controlName + ":" +
                parameterName + ": " + ee);
        }
