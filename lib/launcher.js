...
// is found.
//
function getProgramsList(launcherPermissions, loadedState,
        spew, userName, userFunc = null) {
    ...
    function done() {
        ...
        spew("got programs=" + JSON.stringify(programs));

        // Each time we get the launcher programs list we need to
        // update the launcherPermissions object, because there may be
        // programs added to the web launcher directory at any time.
        Object.keys(opt.users).forEach(function(name) {

            if(name === 'admin') return;

            if(launcherPermissions[name] === undefined)
                launcherPermissions[name] = {};
            Object.keys(programs).forEach(function(path) {
                if(launcherPermissions[name][path] === undefined) {
                    // See if loadedState has a value to override this.
                    try {
                        let val = loadedState.launcherPermissions[name][path];
                        launcherPermissions[name][path] = val;
                    } catch(e) {
                    }
                    // Set the default permission as false.
                    if(launcherPermissions[name][path] === undefined)
                        launcherPermissions[name][path] = false;
                }
            });
        });

        //console.log("launcherPrograms=" + JSON.stringify(launcherPrograms));
        //console.log("launcherPermissions=" + JSON.stringify(launcherPermissions));
    ...
    //
    try {
        dir = fs.realpathSync(opt.launcher_dir);
...
//
// launcherPermissions should be the same in every call,
// launcherPermissions is defined in crts_contestServer.js and is shared
// with this function.
//
// Same for loadedState.
//
function initConnection(io, spew, userName,
    launcherPermissions, loadedState) {

    var saved = true;

    if(launcherPermissions[userName] === undefined &&
        userName !== 'admin')
        launcherPermissions[userName] = { };
    ...
    if(userName === 'admin') {
        ...
        io.On('launcherPermission', function(user, path, permit) {

            try {
                launcherPermissions[user][path] = permit;
                io.BroadcastUser('launcherPermission', 'admin',
                    user, path, permit);
            } catch (e) {
                spew("Failed to set launcherPermission for \"" +
                    user + '" for program "' + path + '" to ' +
                    permit);
            }
            // Tell all admin connections that this may not be saved
            // in the saved state file now.
            checkEmitSaved.markUnsaved(io)
            spew("Set launcherPermission for \"" +
                user + '" for program "' + path + '" to ' +
                permit);
        });
    }
    ...
    io.On('getLauncherPrograms', function() {
        ...
        getProgramsList(launcherPermissions, loadedState, spew,
                userName, function(programs) {
            ...
            if(userName === 'admin') {
                ...
                Object.keys(programs).forEach(function(path) {
                    ...
                    Object.keys(opt.users).forEach(function(name) {
                        if(name === 'admin') return;
                        p[path][name] = launcherPermissions[name][path];
                    });
                });
                io.Emit('receiveLauncherPrograms', programs, p);
                spew("'receiveLauncherPrograms' sending: " + 
                        JSON.stringify(programs) + ",\n" +
                        JSON.stringify(p) + ")");
                return;
            }
            // Else for a regular user we only send the programs they have
            // permission to launch.  We need to figure that out now.
            var p = {};
            //spew(JSON.stringify(launcherPermissions));
            Object.keys(programs).forEach(function(servicePath) {
                if(launcherPermissions[userName][servicePath] === undefined ||
                    launcherPermissions[userName][servicePath] === false)
                    return;
                // We have permission.
                p[servicePath] = programs[servicePath];
            });
            io.Emit('receiveLauncherPrograms', p, {});
            spew("'receiveLauncherPrograms' sending: " + 
                        JSON.stringify(p) + ", {})");
    ...
    function broadcastProgramTally(lProgram) {
        ...
        Broadcast('programTally', lProgram.path, data);
    }
    ...
    function broadcastProgramRunStatus(rp, state) {
        ...
        Broadcast('programRunStatus',
            rp.path,  rp.name, rp.pid, rp.args, state);
    }


    function launch(path_in, args, programName='', opts = {}) {
...
console.log("launcherPrograms=" + JSON.stringify(launcherPrograms));
        ...
        if(userName !== 'admin')
            if(launcherPermissions[userName][path_in] === undefined ||
                    launcherPermissions[userName][path_in] === false) {
                spew("User: " + userName +
                    " does not have permission to run: " +
                    path_in);
                return;
            }
        ...
        spew('User: ' + userName + ' launching: ' + path_in + ' ' + args);
        ...
        if(programName === '') {
            ...
            while(undefined !== programNameToRunningPrograms[programName])
                programName = path_in + '_' + (launchCount++);
        }
...
console.log("                                        args=" + JSON.stringify(arguments));
        ...
        if(Object.keys(launcherPrograms).length < 1)
            // We get the list of programs only if we did not get any
            // programs yet.
            getProgramsList(launcherPermissions,  loadedState,
                spew, userName, function(programs) {
                    launch(path_in, args, programName, opts);
            });
        else
            // We have a list of launcherPrograms that we'll use.
            launch(path_in, args, programName, opts);
    });
    ...
    io.On('signalProgram', function(pid, signalType) {
        ...
        if(userName !== 'admin') {
            // We may have a bad request, so we just try it out and
            // catch the bad request.
            try {
                if(!launcherPermissions[userName][runningPrograms[pid].path])
                    throw "permission denied for user: " + userName;
            } catch(ee) {
                spew("'signalProgram': pid=" + pid + ": " + ee);
                return;
            }
        }
