...
function initConnection(io, spew, userName,
    ...
    // permissions for this userName.
    //
    // We copy the reference, so if we edit to it, it is changed in
    // launcherPermissions; and vise versa.
    var permissions = launcherPermissions[userName];
    ...
    if(userName === 'admin') {
        ...
        io.On('launcherPermission', function(userName, path, permit) {
            ...
            try {
                launcherPermissions[userName][path] = permit;
                io.BroadcastUser('launcherPermission', 'admin',
                    userName, path, permit);
            } catch (e) {
                spew("Failed to set launcherPermission for \"" +
                    userName + '" for program "' + path + '" to ' +
                    permit);
            }
    ...
    io.On('getLauncherPrograms', function() {
        ...
        getProgramsList(opt.launcher_dir, function(programs) {
            ...
            Object.keys(programs).forEach(function(servicePath) {
                if(permissions[servicePath] === undefined ||
                    permissions[servicePath] === false)
                    return;
                // We have permission.
    ...
    io.On('launch', function(path_in, args, programName='', opts = {}) {
        ...
        if(userName !== 'admin')
            if(permissions[path_in] === undefined ||
                    permissions[path_in] === false) {
                spew("User: " + userName +
