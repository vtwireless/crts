...
//
function GetParameterPermission(funcName, user, programName, setGet,
    ...
    console.log("controllers=" + JSON.stringify(controllers[programName][setGet][controlName]));
...
if(opt.radio_port > 0) {
    ...
    function addControllerCB(controller) {
        ...
        try {
            ...
            console.log("controller=" + JSON.stringify(controllers) + ": " + e);
            //return; ???
        }


        // Loop over 'admin' sessions.
        ...
        // Loop over regular users.
        Object.keys(sessions).forEach(function(userName) {

            if(userName === 'admin') return;

            Object.keys(sessions[userName].ios).forEach(function(wsId) {

                // Tell the client about this controller; and all its
                // setters and getters.
                //
                let controllersSent = {};
                controllersSent[controller.programName] = controller;

                userAddControllers(sessions[userName].ios[wsId], userName,
                    controllersSent);
            });
        });
    }
...
// userAddControllers() emits 'addController' to non-admin users the
// setters and getters that the user has permission to access.
//
// Argument _controllers is an like the global controllers but with just
// the controllers in it that we need to sift through and get the setters
// and getters that this user has access to.
//
// This will emit once for each controller that the user has any access
// to.
//
function userAddControllers(io, userName, _controllers) {

    // Note: this function is not called at regular intervals, so the mess
    // that it is will not slow down the service.
    //
    Object.keys(_controllers).forEach(function(programName) {
        // We emit a 'addController' for each program that has a setter or
        // a getter that this user can access.
        var controllerSend = {}; // copies to emit.
        var controller = _controllers[programName];
        var gotOne = false;
        ["set", "get"].forEach(function(type/* = 'set' or 'get'*/) {
            // We'll send only the 'set' and 'get' parameters that this
            // user has access to.  The web user will not even know that
            // the 'set' and 'get' parameters that they do not have access
            // to even exists; unless permissions change and they have an
            // old copy of this list.
            Object.keys(controller[type]).forEach(function(controlName) {
                var control = controller[type][controlName];
                Object.keys(control).forEach(function(parameterName) {
                    var access = GetParameterPermission('userAddControllers',
                            userName, programName, type, controlName,
                                    parameterName);
                    if(!access) return;
                    if(controllerSend === undefined)
                        controllerSend = {};
                    if(controllerSend[type] === undefined)
                        controllerSend[type] = {};
                    if(controllerSend[type][controlName] === undefined)
                        controllerSend[type][controlName] = {};
                    if(controllerSend[type][controlName][parameterName] === undefined)
                        controllerSend[type][controlName][parameterName] = {};

                    var par = {};
                    if(type === 'set') {
                        par.init = controller[type][controlName][parameterName].init;
                        par.min = controller[type][controlName][parameterName].min;
                        par.max = controller[type][controlName][parameterName].max;
                    } // Else 'get' gets an empty object.

                    // Fill in parSend with a deep copy of par.
                    controllerSend[type][controlName][parameterName] =
        ...
        if(gotOne) {
            if(controllerSend.set === undefined)
                controllerSend.set = {};
            if(controllerSend.get === undefined)
                controllerSend.get = {};

            io.Emit('addController', programName, controllerSend.set,
                controllerSend.get, controller.image);
        }
    });
}
...
function onWebSocketConnect(ws) {
    ...
    ws.on('message', function(message) {
        ...
        if(userName !== 'admin')
            userAddControllers(io, userName, controllers);
    });
    ...
    io.On('getParameter', function(programName, controlName,
        ...
        if(!GetParameterPermission("On('getParameter')",
                userName, programName, 'get',
                controlName, parameterName)) {
    ...
    io.On('setParameter', function(programName, controlName,
        ...
        if(!GetParameterPermission("On('setParameter')",
                userName, programName, 'set',
                controlName, parameterName)) {
    ...
    io.On('subscribeToGetter', function(programName, controlName, parameterName) {

        // Note: It can be the case that the program that is feeding this
        // getter does not exist yet.  That should be okay.  We can just
        // log the request and when the program feeding the getter starts
        // it will get the data.
        //
        // Security BUG: A malicious user could use this command to make
        // the server run out of memory.
        controllers[programName].addBrowserGetSubscription(io, wsId,
                controlName, parameterName);
    });
}
...
function addAdminConnection(io, wsId, spew) {
    ...
    io.On('saveState', function(fileName) {
        ...
        try {
            ...
            // Clobber the current loadedState object, which was used to
            // get state that was set in the past, but may not necessarily
            // exist now; so we may be loosing information by saving the
            // current running state.  We need a deep copy that does not
            // have references to parts of the object that we are
            // copying.
            //
            // It is a little inefficient to deep copy like this, but it
            // is fool-proof, and the code is short.  No matter, this
            // function is not called frequently.
            loadedState = {
                controllers: JSON.parse(JSON.stringify(controllers)),
                launcherPermissions: JSON.parse(JSON.stringify(launcherPermissions))
            };
    ...
    io.On('changePermission', function(user, programName, setGet,
        ...
        // This access is also checking that keys to objects exist too.
        // 'admin' has access, but does the access object exist.
        var access = GetParameterPermission("On 'changePermission'",
                'admin', programName, setGet,
                controlName, parameterName);
        if(!access) return;

        // Change our local controllers object.  The above
        // GetParameterPermission() will have verified that this chain of 6
        ...
        // We must remove any existing subscriptions that this user has if
        // they no longer have access.
        if(boolValue) return;

        Object.keys(sessions).forEach(function(user) {
            if(user === 'admin') return;
            Object.keys(sessions[user].ios).forEach(function(wsID) {
                console.log('wsID=' + wsID);
                controllers[programName].removeBrowserGetSubscription(wsID,
                    controlName, parameterName);
            });
        });
    });
    ...
    function setParameterLimit(Type, type, programName, controlName,
        ...
        if(!GetParameterPermission("On('setParameter" + Type + "')",
                'admin', programName, 'set',
                controlName, parameterName)) {
