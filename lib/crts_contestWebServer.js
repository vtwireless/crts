...
function GetUserPermData(funcName, user, programName, setGet,
    ...
    // controllers[programName][setGet][controlName][parameterName][user]
    //
    ...
    // all this "try" checking a simple webSocket send could crash the
    // server.
    //
    ...
    try {
        parameter = controllers[programName][setGet][controlName][parameterName];
        let access = parameter[user];
        //console.log('user "' + user + '" has' + (access?'':' NO') + ' access to ' +
        //    programName + ':' + setGet + ':' + controlName + ':' +
        //    parameterName);
    } catch {
...
function addAdminConnection(io, wsId, spew) {
    ...
    if(controllers) {
        ...
        io.On('changePermission', function(user, programName, setGet,
            ...
            // GetUserPermData() will have verified that this chain of 5
            // keys [][][][][] to this controllers object is valid.

            controllers[programName][setGet][controlName][parameterName][user] = boolValue;
