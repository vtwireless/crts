...
function onWebSocketConnect(ws) {
    ...
    ws.on('message',  function(message) {
        ...
        // The first message must have the user name and password.
        ...
        if(user.name === undefined || user.password === undefined ||
            ...
            user.password !== users[user.name].password)
            return closeFail('invalid user (' + user.name+ ') and/or password (' +
                user.password + ')');

        pre = ws.Prefix = 'User: ' + user.name + ' WebSocket[' + id +
            '](' + ws._socket.remoteAddress + ' :' +
            ws._socket.remotePort + '):';
