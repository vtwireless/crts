...
module.exports = function(req, res, successCallback) {
    ...
    // We disable this authenticate() thing by setting the correct
    // user and password.
    obj.user = 'admin';
    obj.password = users['admin'].password;
    ...
    setTimeout(function() {
        ...
        if(success) {
            ...
            // token.  Note we keep is simple because there will be no
            // added security by encrypting the token on an encrypted
            // connection.  We just need the password to be is do random and
