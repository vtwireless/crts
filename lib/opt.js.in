...
var options = {
    ...
    debug: {
        ...
        type: 'bool',
        // We get the DEBUG CPP macro value and test if it's
        // defining DEBUG as a CPP macro.  If it is that overrides this
        // option as being set where-by making this "on" whither the
        // argument was set of not.  We call it a debug build.
        default_value: ("@DEBUG@".substr(0,7) === '#define'),
    ...
    mqtt_port: {
        ...
        default_value: 7111,
        argument: 'PORT',
        help: 'run a MQTT broker server using port PORT.  Setting PORT to ' +
            '0 will make it not run a MQTT broker server.'
    },
    ...
    radio_port: {
        ...
        argument: 'PORT',
        help: 
            'THIS OPTION IS GOING AWAY.  IT IS DEPRECIATED. ' +
            ...
            'WebRTC, but building the WebRTC C++ API is a total pain ' +
            'in the ass; time is money.  ' +
            'TODO: Can WebRTC use the same listening port as HTTP; ' +
    ...
    keys.forEach(function(key) {
        ...
        if(typeof(opt.default_print) === 'undefined' && opt.default_value) {
            if(opt.type === 'string')
                ...
                // being an option.
                //
                if(opt.default_value !== undefined && opt.default_value) {
                    var default_value = ' This is set in opt.js.  ' +
                        'You need to edit opt.js to change that.';
                } else {
