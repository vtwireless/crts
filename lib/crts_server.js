...
// Add more configuration stuff to opt.
//
if(opt.tls !== undefined) {
    //
    // TODO: Change this if you add TLS key setting options.  We use the
    // same public/private TLS key pair for both the https and MQTT parts
    // of the server, hence we share them between the https server and the
    // MQTT server via opt in the code below this block.
    //
    console.log("TLS keys are set in opt.js");
    process.exit(1);
}
// else
...
opt.tls = {
    key : path.join(keys_dir, 'private_key.pem'),
    cert: path.join(keys_dir, 'public_cert.pem')
};
