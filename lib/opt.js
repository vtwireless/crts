...
const default_contest_dir = path.join(os.homedir(), 'crts_contest_PID#');
...
var options = {
    ...
    contest_dir: {
        type: 'string',
        default_value: default_contest_dir,
        help: 'set the directory where generated files are stored.',
    },

    contest_dir_overwrite: {
        type: 'bool',
        help: 'if the "contest_dir" exists at the start of this program,' +
...
if(opt.contest_dir === default_contest_dir)
    opt.contest_dir = path.join(os.homedir(),
        'crts_contest_' + process.pid.toString());

// We need the full path:
opt.contest_dir = path.resolve(opt.contest_dir);
...
if(opt.contest_dir_overwrite && fs.existsSync(opt.contest_dir)) {
    console.log("Removing old contest directory: " + opt.contest_dir);
    rmDirRecursive(opt.contest_dir);
}

assert(fs.existsSync(opt.contest_dir) === false,
    "Contest directory \"" + opt.contest_dir + "\" exists");

// Make (or remake) the contest directory.
fs.mkdirSync(opt.contest_dir, { recursive: true });

assert(fs.existsSync(opt.contest_dir),
    "Failed to make contest directory: " + opt.contest_dir);
console.log("Made contest directory: " + opt.contest_dir);
