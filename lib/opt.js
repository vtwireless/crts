...
const default_doc_root = path.normalize(path.join(__dirname, "..", "htdocs"));
...
var options = {
    ...
    doc_root: {
        type: 'string',
        default_value: default_doc_root,
        help: "set the static web pages document root."
    },
    ...
    contest_dir: {
        ...
        default_value: default_doc_root,
        argument: 'PATH',
        help: 'set the real directory to find file from URL paths that ' +
            'start with /contest/.  This contest directory will contain ' +
            'files that served in the web pages for this partiular ' +
            'running of this web server.  Other contests can have other ' +
            'web pages served.'
    },
    ...
    launcher_dir: {
        ...
        help: 'set the directory to search for programs to launch.  ' +
            'This web launcher directory contains the programs that ' +
            'user of this web server program will run on behalf of ' +
            'connected browser clients.  The person running this server ' +
            'may add or remove programs from this directory.  ' +
            'Programs may be added to this directory while the server is ' +
            'running, and then browsers will see the programs in page ' +
            'reloads, just like other HTTP served files on most web servers.'
    },

    state_file: {

        type: 'string',
        default_value: 'server_state.json',
        argument: 'PATH',
        help: 'load this JSON file at startup.  If a relative path to the' +
            ' file is given, then the path will be relative to the launcher' +
            ' directory.  If at startup the file is not found, an empty' +
            ' state file' +
            ' will be created.  This will be the default file that will be' +
            ' used to save the state of the server.  Care must be used in' +
            ' order to not loose your work in setting up your contest' +
            ' service by clobbering this file.'
    },
    ...
    server_address: {
        ...
        help: 'set the address or domain name that refers to this ' +
            'server from computers (nodes) that access the stream programs ' +
            'that access the USRPs.' + 
...
// keys is an array of strings like key[n] === "radio_port"
...
for(var i=2; i < alen; ++i) {
    ...
    for(var k=0; k<keys.length; ++k) {
        ...
        // Note: this does not look for short form of options like in
        // commands like "ls -al".
        ...
        // Option not found yet, continue looping over option keys.
...
try {
    ...
    console.log('The launch directory: ' + opt.upload_dir +
        ' does not exist yet.');
};
...
try {
    ...
    if(!fs.lstatSync(opt.launcher_dir).isDirectory()) {
        ...
        // We are screwed.  Ya, this directory could be
        // made later, but it exists as a file that is not
        // a directory, so we'll bail in this case.
        process.exit(1);
    }
} catch(e) {
    console.log("The launcher directory \"" +
...
// The state file can change as the server runs.  It just starts at the
// opts value, but admin clients can change it at run time.  We started
// with the state being stringified form of the controllers object, but we
// may add more to that.
//
// TODO: port from UNIX dir string to windoz
//
if(opt.state_file.substr(0,1) !== '/')
    opt.state_file = path.join(opt.launcher_dir, opt.state_file);
